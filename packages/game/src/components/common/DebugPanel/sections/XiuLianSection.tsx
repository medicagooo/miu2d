...
export const XiuLianSection: React.FC<XiuLianSectionProps> = ({
  ...
  // 没有等级数据的武功不能升级
  const growthState = getMagicGrowthState(xiuLianMagic.magic, xiuLianMagic.level);
  const canUpgrade = growthState !== 'unconfigured';
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-[10px] text-[#969696]">
            {canUpgrade
              ? `等级 ${xiuLianMagic.level} / ${xiuLianMagic.magic.maxLevel || 10}（${magicGrowthLabel(growthState)}）`
              : `等级 ${xiuLianMagic.level}（无成长配置）`}
          </div>
        </div>
