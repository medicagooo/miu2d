...
- 当前需求：只读使用现有远程公开游戏 API/S3 资源；修复本地 Docker 配置；在 Debian WSL2 部署本地服务，并确保 `demo`、`sword1`、`sword2` 可从 Windows 浏览器进入；根据 `demo` 的 NPC 武功效果制作同画风图标资产与键名清单，将 34 枚 PNG 和武侠风格说明作为对应 NPC 武功的本地回退接入游戏与后台展示，并让 34 门有效 NPC 武功同时进入玩家可学习/可添加目录；不得写入远程数据库或 S3。
- 需求变更记录：
