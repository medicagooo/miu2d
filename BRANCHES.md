...
- 当前需求：只读使用现有远程公开游戏 API/S3 资源；修复本地 Docker 配置；在 Debian WSL2 部署本地服务，并确保 `demo`、`sword1`、`sword2` 可从 Windows 浏览器进入；根据 `demo` 的 NPC 武功效果制作同画风图标资产与键名清单，将 34 枚 PNG 和武侠风格说明作为对应 NPC 武功的本地回退接入游戏与后台展示，并让 34 门有效 NPC 武功同时进入玩家可学习/可添加目录；调试物品选择器必须按事件物品的实际使用脚本分类，执行有效 `AddMagic(...)` 的物品归入“秘籍”，其余保持“事件”；不得写入远程数据库或 S3；完成验证后将本分支全部修改普通推送至同名远程分支。
- 需求变更记录：
  ...
  - 2026-09-01：用户确认补充 34 门原 NPC 武功的润色说明；按已验证配置键提供本地 `intro` 展示覆盖，确保全部采用确认后的润色文案，不包含帧数；未知武功保留 API 原文，不修改远程数据。
  - 2026-09-01：用户要求修正调试物品分类；以 `script/goods` 中实际执行的有效 `AddMagic(...)` 命令为唯一秘籍判据，不按文件名或物品名猜测；当前 `demo` 预期 13 件秘籍、23 件事件，完成后推送本分支全部修改。
- 是否已合并到 main：否
- 操作记录：
  ...
  - 2026-09-01：拟将 34 条 NPC 武功说明回退接入引擎武功数据和后台选择器预览并提交到当前本地分支；提交后快进同步至 Debian WSL 的 `/home/medic/projects/miu2d`，仅重建 `miu2d-web` 并验证玩家学会原 NPC 武功后可见说明；不修改数据库、MinIO 或远程 S3，不推送远程。
  - 2026-09-01：拟在当前分支实现调试物品秘籍分类：只读加载事件物品的 `script/goods` 使用脚本，忽略注释并按有效 `AddMagic(...)` 命令分类；完成测试后快进同步至 Debian WSL 的 `/home/medic/projects/miu2d`，仅重建 `miu2d-web` 并验证 `demo` 中秘籍 13 件、事件 23 件；不修改数据库、MinIO 或远程 S3。
  - 2026-09-01：拟将当前本地分支 `0901-fix-docker-wsl-deploy` 的全部已提交修改以普通推送方式发布到 `origin/0901-fix-docker-wsl-deploy`（`git@github.com:medicagooo/miu2d.git`），并设置该远程分支为上游；推送后仍保持“是否已合并到 main：否”。
