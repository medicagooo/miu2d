...
![真实光影](packages/web/public/screenshot/real-lighting.png)
...
- **完全控制渲染循环** — `SpriteBatcher` 将约 4,800 张地图瓦片合并为 1–5 次 WebGL 绘制调用；`RectBatcher` 将约 300 个天气粒子归为 1 次调用；真实光影 pass 将逐实体加法光晕与基于 SHD 的阴影渲染合成输出
...
| **后端** | `@miu2d/server` | Hono + tRPC + Prisma ORM · 21 张 PostgreSQL 表 · 19 个路由 |
...
| 前端 | React 19 · Vite 8 (rolldown) · Tailwind CSS 4 |
...
| 系统 | 模块 | 要点 |
|------|------|------|
| **渲染系统** | `renderer/` | 原生 WebGL 精灵批处理（约 4,800 瓦片 → 1–5 次绘制调用）、Canvas2D 回退、GLSL 颜色滤镜（中毒 / 冰冻 / 石化）、屏幕特效（淡入淡出、闪光、水波纹）、**真实光影（加法光晕效果、逐实体发光、基于 SHD 的阴影渲染）** |
