...
$md = @('# 武功补图清单', '', '来源：2026-09-08 公开游戏接口，范围为原始 icon 为空的 94 个配置。65 张新绘、29 项复用（27 新绘 + 1 原图 + 1 既有备用图）。', '', '新剑侠情缘同名复用月影传说，同名多配置优先相同 key。弓箭原始资源 404，因此复用已有备用图；推山填海保留原始 30×40 第一帧。数值、说明、归属和原有非空 icon 不变。未推送、未部署。', '', '本目录 index.html 可浏览、搜索和放大图片。manifest.json 含数据依据、资产路径、复用来源和 SHA256；prompts.json 保留生成设计及未采用提示词。', '')
